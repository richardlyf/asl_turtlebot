...
class EkfLocalization(Ekf):
    ...
    def measurement_model(self, z_raw, Q_raw):
        ...
        # Everything is already vectorized in compute_innovations
        z = v_list.reshape(-1)
        Q = scipy.linalg.block_diag(*Q_list)
        H = H_list.reshape(-1, H_list.shape[2])
        ########## Code ends here ##########
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        J = hs.shape[1]
        # (I, J, 2)
        z_matrix = np.repeat(z_raw.T[:, np.newaxis, :], J, axis=1)
        hs_matrix = np.repeat(hs.T[np.newaxis, :, :], I, axis=0)
        alpha_diff = angle_diff(z_matrix[:, :, 0], hs_matrix[:, :, 0])
        r_diff = z_matrix[:, :, 1] - hs_matrix[:, :, 1]
        v = np.dstack((alpha_diff, r_diff))
        # (J, 2, 3) J is batch size
        Hs_cube = np.dstack(Hs).transpose((2, 0, 1))
        # (J, 3, 2)
        Hs_cube_T = Hs_cube.transpose(0, 2, 1)
        # (J, 2, 2)
        Sj = np.matmul(Hs_cube, np.matmul(self.Sigma, Hs_cube_T))
        Q_batched = np.dstack(Q_raw).transpose((2, 0, 1))
        # (I, J, 2, 2)
        S = np.repeat(Sj[np.newaxis, ...], I, axis=0) + np.repeat(Q_batched[:, np.newaxis, ...], J, axis=1)
        v_t = v[..., np.newaxis, :]
        # (I, J)
        d = np.squeeze(np.matmul(v_t, np.matmul(np.linalg.inv(S), v[..., np.newaxis])), axis=(2,3))
        d_min = np.min(d, axis=1)
        js = np.argmin(d, axis=1)
        i_indices = np.where(d_min < self.g ** 2)[0]
        j_indices = js[i_indices]
        v_list = v[i_indices, j_indices]
        Q_list = Q_batched[i_indices]
        H_list = Hs_cube[j_indices]
        ########## Code ends here ##########
...
class EkfSlam(Ekf):
    ...
    def transition_model(self, u, dt):
        ...
        g_pose, Gx_pose, Gu_pose = tb.compute_dynamics(self.x[:3], u, dt)
        g[:3] = g_pose
        Gx[:3, :3] = Gx_pose
        Gu[:3, :2] = Gu_pose
        ########## Code ends here ##########
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        # Exactly the same as EkfLocalization.compute_innovations()
        I = z_raw.shape[1]
        J = hs.shape[1]
        # (I, J, 2)
        z_matrix = np.repeat(z_raw.T[:, np.newaxis, :], J, axis=1)
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            if j >= 2:
                ...
                x_cam_rotated, y_cam_rotated = np.dot(Rz, self.tf_base_to_camera[:2])
                x_cam = x_cam_rotated + x_base
                y_cam = y_cam_rotated + y_base
                cam_coord_norm = np.linalg.norm(np.array([x_cam, y_cam]))
                Hx[1, idx_j] = -cam_coord_norm * np.sin(np.arctan2(y_cam, x_cam) - alpha)
            ########## Code ends here ##########
